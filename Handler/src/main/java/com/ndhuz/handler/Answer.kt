 ...
 * https://juejin.cn/post/6844903683252191246
 *
 * /// 描述整个消息机制运行：
 * 1、首页系统调用 ActivityThread 中 main 函数初始化主线程的 Looper，并把 Looper 保存在 ThreadLocal 中 (保证一个线程只会有一个 Looper)
 * 2、然后调用 Looper.loop() 进入一个死循环，一直循环调用 MessageQueue.next() 获取下一个 Message，
 * 3、MessageQueue.next() 中也有一个死循环用于获取队列中的下一条 Message，在获取不到消息或者消息执行时间未到时会调用 native 层 NativeMessageQueue 的休眠方法，
 ...
 * 2、泄漏的时机在于发送了一条延时的 Message，但宿主，比如 Activity，已经被回调了 onDestroy()，此时却因为 Message 还没有执行发生内存泄漏
 ...
 * 2、在 Activity 被摧毁时更建议调用 Handler.removeCallbacksAndMessages(null) 把相关联的 Message 清空
 *
 *
 ...
 *   NativeMessageQueue 调用了 native 层的 Looper，该 Looper 会使用 Linux 的 epoll 机制进行休眠，休眠时会让出 CPU 调度
