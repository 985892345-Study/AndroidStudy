...
* 如果不是自动测量模式时，则会直接调用 LayoutManager.onMeasure() 进行测量，一般出现在自定义 LayoutManager 时
...
*   2.2. 正常布局填完屏幕后，如果一级缓存 Scrap 中有多的 ViewHolder，会全部布局上去 (由 LayoutManager 实现)
...
*
* /// Rv 动画的实现 ?
* Rv 中有一个 ViewInfoStore 工具类专门记录 item 的位置信息
* 首先在 dispatchLayoutStep1() 中保存旧的 item 位置信息和新增的 item 的信息
* 然后在 dispatchLayoutStep3() 中保存布局后的 item 信息，
* 最后调用 mItemAnimator 执行动画
* 官方默认的 ItemAnimator 通过调用 view 自带的 animate() 设置 translationX、translationY 来实现动画
* (view.animate() 是官方封装的一种属性动画，可以用于更方便的实现动画)
* */